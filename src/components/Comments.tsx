  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {status === "loading" && (
          <div
            style={{ fontSize: 11, color: "var(--text-dim)", padding: "4px 0" }}
          >
            加载中…
          </div>
        )}
        {status === "offline" && (
          <div
            style={{ fontSize: 11, color: "var(--sand-dim)", lineHeight: 1.6 }}
          >
            留言功能需部署到 Vercel 并连接 KV / Upstash Redis Store 后启用。
            <br />
            本地调试请用{" "}
            <code style={{ fontFamily: "monospace" }}>vercel dev</code>。
          </div>
        )}
        {status === "error" && (
          <div style={{ fontSize: 11, color: "#e07040" }}>
            加载失败: {error ?? "unknown"}
          </div>
        )}
      ...
      >
        ...
        >
          <span style={{ fontSize: 10, color: "var(--text-dim)" }}>
            {error ? (
              <span style={{ color: "#e07040" }}>{error}</span>
            ) : (
              `${message.length}/500`
            )}
          </span>
