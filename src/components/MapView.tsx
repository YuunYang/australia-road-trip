...
export default memo(function MapView({ selectedDay, onDaySelect }: Props) {
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, {
      ...
      minZoom: 4,
      maxZoom: 14,
      // Loose bounds around the Sydney→Melbourne corridor — user can pan freely
      // within reason but we won't lazy-fetch tiles halfway around the globe.
      maxBounds: L.latLngBounds([-42.0, 143.0], [-30.0, 154.5]),
    ...
    // Base tiles — no labels + no retina. ~4x less pixel data on high-DPI phones
    // than `dark_all` with `{r}`, and the tiles themselves are smaller (no text).
    L.tileLayer(
      "https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}.png",
      {
        maxZoom: 14, // road-trip overview doesn't need street-level zoom
        minZoom: 4,
        updateWhenIdle: isTouch, // don't refetch tiles while panning on mobile
        keepBuffer: 1,
      },
    ).addTo(map);

    // Labels — loaded lazily, only kick in when the user zooms past level 8.
    // At the default overview zoom (7) this layer fetches zero tiles.
    L.tileLayer(
  ...
  useEffect(() => {
    ...
    if (animRouteRef.current) {
      map.removeLayer(animRouteRef.current);
      animRouteRef.current = null;
    }
    ...
    animRouteRef.current = L.polyline(
      [
        [day.from.lat, day.from.lng],
        [day.to.lat, day.to.lng],
      ],
